...
@section('page_title', 'Manajemen Brand Mobil')
@section('breadcrumb', 'tambah, edit, atau hapus merk mobil')
                ...
                <div class="card border-0 shadow-sm rounded-3 sticky-top" style="top: 100px;">
                    <div class="card-header bg-white py-3 border-bottom">
                        <div class="fw-bold d-flex align-items-center text-dark">
                            <i class="bi bi-tag-fill me-2 text-primary"></i>Tambah Brand Baru
                        </div>
                    </div>
                    <form method="POST" action="{{ route('admin.mobil.brand.store') }}">
                        @csrf
                        <div class="card-body p-4">
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="form-label fw-semibold small text-secondary">Nama Brand <span
                                            class="text-danger">*</span></label>
                                    <input name="nama_brand" type="text" class="form-control rounded-3 py-2"
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        <span class="fw-bold text-dark"><i class="bi bi-list-ul me-2 text-primary"></i>Daftar Brand</span>
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody class="small">
                                @forelse ($brands as $item)
                                    <tr>
                                        <td class="text-dark fw-medium ps-4">{{ $item->nama_brand }}</td>
                                        <td class="text-muted">{{ $item->deskripsi ?? '-' }}</td>
                                        <td class="pe-4">
                                            <div class="d-flex justify-content-center gap-2">

                                                <a href="{{ route('admin.mobil.brand.edit', $item->id) }}"
                                                    class="btn btn-sm btn-info bg-opacity-10 border-0 text-info px-2 py-1 rounded-2"
                                                    title="Edit">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <form action="{{ route('admin.mobil.brand.destroy', $item->id) }}"
                                                    method="POST"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-4">Belum ada data brand
                                            terdaftar.</td>
                                    </tr>
                                @endforelse
                            </tbody>
