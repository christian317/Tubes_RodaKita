    ...
    <nav class="navbar navbar-expand-lg bg-white border-bottom shadow-sm sticky-top py-3">
        <div class="container">
            ...
            <a class="navbar-brand d-flex align-items-center gap-2" href="{{ route('pelanggan.dashboard') }}">
                <div class="bg-primary text-white rounded-3 d-flex align-items-center justify-content-center fw-bold"
                    style="width: 38px; height: 38px; font-size: 1.1rem;">RK</div>
            ...
            <div class="collapse navbar-collapse" id="navbarPelanggan">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4 fw-medium">
                    ...
                    {{-- MENU KATALOG MOBIL --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pelanggan.dashboard') || request()->routeIs('pelanggan.mobil.*') || request()->routeIs('pelanggan.order.*') ? 'active text-primary fw-bold' : 'text-dark' }}"
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pelanggan.riwayatBooking.*') ? 'active text-primary fw-bold' : 'text-dark' }}"
                            href="{{ route('pelanggan.riwayatBooking.index') }}">
                            Riwayat Booking
                        </a>
                    </li>
                    ...
                    {{-- MENU JADWAL LIBURAN --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pelanggan.jadwal.*') ? 'active text-primary fw-bold' : 'text-dark' }}"
                            href="{{ route('pelanggan.jadwal.index') }}">
                            Jadwal Liburan
                        </a>
                    </li>
                ...
                <div class="d-flex align-items-center gap-3 mt-3 mt-lg-0">
                    <div class="d-flex align-items-center gap-2">
                        <div class="rounded-circle bg-primary bg-opacity-10 text-primary d-flex justify-content-center align-items-center fw-bold"
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="m-0">
                        @csrf
                        <button type="submit"
                            class="btn btn-light border btn-sm rounded-3 text-danger fw-semibold px-3">
